...
{
public:
  explicit LineaPedido(unsigned precio,double cantidad=1):precio_(precio),cantidad_(cantidad){}
...
public:
  ...
  const ItemsPedido& detalles (Pedido& U);
  Pedidos ventas(Articulo& A);
  void mostrarDetallesPedidos();                  
private:
