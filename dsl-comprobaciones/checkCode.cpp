...
bool checkCode::findClass(string className) {
  ...
  if(fileClass == ""){
    ...
    for(list<string>::iterator pos = listSources.begin(); pos != listSources.end(); pos++){
      posAux = *pos;
...
// ----------------------- CHECK CONSTRUCTORS ---------------------
void checkCode::defaultConstructor(string className, bool exist, string notFoundMessage){
  e.setDefaultConstructors(className, exist, notFoundMessage);
  Finder.addMatcher(defaultConstructor_Matcher(className), &e);
}

void checkCode::copyConstructor(string className, bool exist, string notFoundMessage){
  e.setCopyConstructors(className, exist, notFoundMessage);
  Finder.addMatcher(copyConstructor_Matcher(className), &e);
}

void checkCode::moveConstructor(string className, bool exist, string notFoundMessage){
  e.setMoveConstructors(className, exist, notFoundMessage);
  Finder.addMatcher(moveConstructor_Matcher(className), &e);
}

void checkCode::listInitializerConstructor(string className, vector<string> parameters, vector<string> initializers, string message){
...
void checkCode::destructorDeleteMember(string className, vector<string> memberNames, string message){
  for (unsigned int i = 0 ; i < memberNames.size(); i++){
  	e.setDeleteMembers(className, memberNames[i], message);
  	Finder.addMatcher(destructorDeleteMember_Matcher(className, memberNames[i]), &e);
  }
}

void checkCode::numberOfConstructors(string className, unsigned int constructors, bool lenient, string message){
...
void checkCode::staticVariable(string className, vector<string> variableNames, vector<string> constant, string message){
...
void checkCode::memberVariableAccessLevel(string className, vector<string> memberNames, string level, string message){
  for (unsigned int i = 0 ; i < memberNames.size(); i++){
        ...
        Finder.addMatcher(memberVariableAccessLevel_Matcher(className, memberNames[i]), &e);
  }
}
...
// ----------------------- CHECK METHODS ---------------------
void checkCode::method(vector<string> methodNames, vector<vector<string> > parameters, string className, vector<string> constant, string message)
{
  if(methodNames.size() == parameters.size()){
	  for (unsigned int i = 0 ; i < methodNames.size(); i++){
	  	e.setMethodsWithName(methodNames[i], className, parameters[i], constant[i], message);
...
void checkCode::noExceptMethod(vector<string> methodNames, vector<vector<string> > parameters, string className, vector<string> constant, string message){

  if(methodNames.size() == parameters.size()){
	for (unsigned int i = 0 ; i < methodNames.size(); i++){
	  	e.setNoExceptMethods(methodNames[i], parameters[i], className, constant[i], message);
...
void checkCode::inlineMethod(vector<string> methodNames, vector<vector<string> > parameters, string className, vector<string> constant, string message){

  if(methodNames.size() == parameters.size()){
  	for (unsigned int i = 0 ; i < methodNames.size(); i++){
		e.setInlineMethods(methodNames[i], parameters[i], className, constant[i], message);
...
void checkCode::defaultArgumentsInMethod(vector<string> methodNames, vector<vector<string> > parameters, string className, vector<string> constant, vector<unsigned int> numDefaultArgs, vector<vector<string> > defaultArgs, string message){

  if(methodNames.size() == parameters.size()){
  	for (unsigned int i = 0 ; i < methodNames.size(); i++){
		if(numDefaultArgs[i] == 0 || numDefaultArgs[i] == defaultArgs[i].size()){
			e.setDefaultArgumentsInMethod(methodNames[i], parameters[i], className, constant[i], numDefaultArgs[i], defaultArgs[i], message);
...
void checkCode::defaultedMethod(vector<string> methodNames, vector<vector<string>> parameters, string className, vector<string> constant, string message) {

  if(methodNames.size() == parameters.size()){
    for (unsigned int i = 0 ; i < methodNames.size(); i++) {
      e.setDefaultedMethods(methodNames[i], className, parameters[i], constant[i], message);
...
// ----------------------- CHECK OPERATORS ---------------------
void checkCode::copyAssignmentOperator(string className, bool exist, string message){
  e.setCopyAssignmentOperators(className, exist, message);
  Finder.addMatcher(copyAssignmentOperator_Matcher(className), &e);
}

void checkCode::moveAssignmentOperator(string className, bool exist, string message){
...
void checkCode::functionWithReferencedFunction(vector<string> mainFunctionNames, vector<vector<string> > mainFunctionParameters, vector<string> usedFunctionNames, vector<vector<string> > usedFunctionParameters, string message){
  ...
  if(mainFunctionNames.size() == mainFunctionParameters.size() && usedFunctionNames.size() == usedFunctionParameters.size() && mainFunctionNames.size() == usedFunctionNames.size()){  
	  ...
	  for (unsigned int i = 0 ; i < mainFunctionNames.size(); i++){
		...
		Finder.addMatcher(functionWithReferencedFuntion_Matcher(mainFunctionNames[i], usedFunctionNames[i]), &e);
	  }
  }
  else{
...
void checkCode::fileIncludedHeader(string headerName, vector<string> fileNames, string message){
...
void checkCode::friendFunction(vector<string> functionNames, vector<vector<string> > parameters, string className, string message)
{
  if(functionNames.size() == parameters.size()){
          for (unsigned int i = 0 ; i < functionNames.size(); i++){
        ...
        llvm::outs() << "checkCode::friendFunction => different number of functionNames and parameters.\n";
  }
}

void checkCode::friendClass(string className, vector<string> friendClassesNames, string message)
{
